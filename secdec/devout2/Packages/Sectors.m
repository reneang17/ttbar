...
ints=f //.{Power[a_,l_.(m_.+n_. ap)]:>Power[a,l*m]paw[a^(l* n)]}
       //.{Power[a_,l_.(m_.+n_. ep)]:>Power[a,l*m]pew[a^(l*n)]};
...
ints=ints//.{Sq[a0_]* Sq[a1_] :>Sq[a0 a1]};
...
Block[{ints,auxrep={x1->x[1],x2->x[2],x3->x[3],x4->x[4],x5->x[5],x6->x[6]}},

  ints = (integrals /. Int[f_] :> Collect[f, {ep, ap}, Int])/.auxrep;
