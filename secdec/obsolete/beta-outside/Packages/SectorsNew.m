 ...
 ******************************************************************************)
 ...
 Print["SectorsNEW 1.0"];
 Print["created by M. Czakon and adapted for NNLO ttbar by S. Sapeta & R. Angeles"];
 Print["last modified 26 Jun May 2017"];
 ...
 BeginPackage["SectorsNew`", {"Symbols`"}]
...
AlphaEpsilonExpandMod[integrals_, norm_, order_] :=Block[{ints},
 ints=integrals;
 ...
 ******************************************************************************)
 ...
 Block[{ints},
    ...
    ints=f //.{Power[a_,m_.+n_. ap]:>Power[a,m]paw[a^n]}//.{Power[a_,m_.+n_. ep]:>Power[a,m]pew[a^n]};
       ints=ints//.{Power[y_,-1/2]:>Sqi[1/y]}//.{Power[y_,1/2]:>Sqi[y]}
       /. {Power[y_, a_] :>Sqi[y]*y^IntegerPart[a] /; FractionalPart[a] === 1/2}
       /. {Power[y_, a_] :>Sqi[1/y]*y^IntegerPart[a] /; FractionalPart[a] === -1/2};

     (*  ints=ints//.{Sqi[a_](Sqi[a1_] c_+Sqi[a2_] d_):>Sqi[a a1]c+Sqi[a a2]d};*)
 	  ints=ints//.{Sqi[a_](Sqi[a1_] c_+ d_.):>Sqi[a a1]c+Sqi[a]d};

   (*  ints=ints//.{paw[a_](paw[a1_]c_+paw[a2_]d_):>paw[a a1]c+paw[a a2]d};*)
 	  ints=ints//.{paw[a_](paw[a1_]c_+d_.):>paw[a a1]c+paw[a]d};

       (*ints=ints//.{pew[a_](pew[a1_]c_+pew[a2_]d_):>pew[a a1]c+pew[a a2]d};*)
       ints=ints//.{pew[a_](pew[a1_]c_+d_.):>pew[a a1]c+pew[a]d};

       ints=ints//.{paw[a_]paw[b_]:>paw[a b]}//.{pew[a_]pew[b_]:>pew[a b]}//.{Sqi[a_]Sqi[b_]:>Sqi[a*b]};

     (*ints=ints/.{a_.*paw[x_]pew[y_]:>(a//Simplify)*paw[x//Simplify]*pew[(y//Simplify)]};
     ints=ints/.{a_.*pew[x_]/;FreeQ[a,paw]:>(a)*pew[x//Simplify]]};
     ints=ints/.{a_.*paw[x_]/;FreeQ[a,pew]:>(a)*paw[x//Simplify]]};

       ints=ints//.{a_. paw[x_]*pew[y_]+b_*paw[x_]*pew[y_]:>(a+b)*paw[x]*pew[y]};
       ints=ints//.{a_.*pew[y_]+b_*pew[y_]:>(a+b)*pew[y]};*)

 	  ints=ints//.{c_(a_.*Sqi[z_]*pew[y_]+b_):>c*a*Sqi[z]*pew[y]+c*b};

       ints=ints/.{paw[x_]:>Exp[ap Ln[x]],pew[x_]:>Exp[ep Ln[x]]}//.{Sqi[z_]:>Sq[z]};
       ints=ints//.{Sq[a_]Sq[b_]:>Sq[a*b]};
     ...
     (*  GroupLogs[f_] :=
       Block[{ints},
         ...
         (*  ints=ints//.{paw[a_](paw[a1_]c_+paw[a2_]d_):>paw[a a1]c+paw[a a2]d};*)
             ...
             ints=ints//.{a_.* Sqi[z_] paw[x_]*pew[y_]+b_.*Sqi[z_]*paw[x_]*pew[y_]:>(a+b)*Sqi[z]*paw[x]*pew[y]};
             ints=ints//.{a_.*Sqi[z_]*pew[y_]+b_.*Sqi[z_]*pew[y_]:>(a+b)*Sqi[z]*pew[y]};

       	  ints=ints//.{c_(a_.*Sqi[z_]*pew[y_]+b_):>c*a*Sqi[z]*pew[y]+c*b};

             (*ints=ints//.{a_.*Sqi[z_]pew[y_]:>(a//Simplify)*Sqi[z]*pew[(y//Simplify)]};*)
