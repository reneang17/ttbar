...
BeginPackage["GeneratorUnitKL`", {"MappingsAndCounting`","Symbols`"}];

Print["GeneratorUnitKL"];
...
(*Sets coords that satisfy the on-shell and observable condition*)
...
maindir=DirectoryName[FindFile["GeneratorUnitKL.m"]];
...
Module[{rawcoords,unitlkplus,unitklminus,unitlk,MappedCoordsTemp,per,AuxVec,AuxSeq,c1,c0},

(*Meaning rawcoords:                                  *
 *  {  ( ,l^1/ l^0 ,lt_(d-3),lt_(d-4) )/ l^0          *
 * ;  (1 ,kt(d-3))  }                                 *)

rawcoords = {1, (lp - lm)/(lp + lm), lT/(2 (lp + lm)) Cos[phi],
   lT/(2 (lp + lm)) Sin[phi], 1, (kp - km)/(kp + km)} ;

unitklplus=EvDeltas[#] & /@ rawcoords;
CPrint[unitklplus];
unitklminus=EvDeltas[#] & /@ rawcoords;

SetDirectory[ToString[maindir]];
CreateDirectory[ToString[OnShellUnitkl]];

SetDirectory[ToString[maindir]<>ToString[OnShellUnitkl]];


        Write[ToString[OnShellUnitkl]<>".dat",
        unitklplus];
        Close[ToString[OnShellUnitkl]<>".dat"];


Print["Unit coordinates written"];
