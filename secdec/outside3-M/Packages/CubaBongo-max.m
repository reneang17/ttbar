...
Module[{ListForMC,collectedtotal,collectedint,collectedintsum,finalints,out,
  IntOptions,MCTime,finalexpression,NSec,ic,jc,rc,
...
Close[ToString[adatafile]<>"_G.dat"];
  ...
  collectedtotal=Flatten[ListForMC/.{be->0}];
...
SetDirectory["./"<>ToString[adatafile]<>"_ap_ep_"<>ToString[ic]<>"_"<>ToString[jc]<>"_C"];
Do[  
aux={};
ApenndTo[aux,
Coefficient[Coefficient[GlobalConst*collectedtotal[[rc]],ap,-ic],ep,-jc] /.{Int[ax_,___]:>ax}];

,{rc,1,Length[collectedtotal]}]

write[tostring[adatafile]<>".dat",(Plus@@aux)//Cform];
close[tostring[adatafile]<>tostring[rc]<>".dat"];
