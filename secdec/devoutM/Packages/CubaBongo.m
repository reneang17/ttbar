...
Module[{ListForMC,collectedtotal,collectedint,collectedintsum,finalints,out,
  IntOptions,MCTime,finalexpression,NSec,ic,jc,
...
Do[
  Do[
    AppendTo[ EvaMath , ap^ic *  ep^jc *
    Coefficient[Coefficient[GlobalConst * auxtotal ,ap,ic],ep,jc]
    ];
  ,{ic,-2,0}]
,{jc,-3,0}];

  Write[ToString[adatafile]<>"_EWS.dat", EvaMath];
...
aux ={ Coefficient[Coefficient[GlobalConst*auxtotal,ap,-ic],ep,-jc] /.{Int[ax_]:>ax}};
aux=aux//.{W[args__]*a1_.+W[args__]* b1_:> W[args] (a1+b1),
           WW[args__]*a1_.+W[args__]* b1_:> WW[args] (a1+b1)};

CreateDirectory[ToString[adatafile]<>"_ap_ep_"<>ToString[ic]<>"_"<>ToString[jc]<>"_C"];

            If[aux=!={},
              SetDirectory[PathMainDir<>"/"<>
              ToString[adatafile]<>"_ap_ep_"<>ToString[ic]<>"_"<>ToString[jc]<>"_C"];
