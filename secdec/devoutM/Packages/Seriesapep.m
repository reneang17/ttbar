...
Print["regints",Length[intlist[[jc]][[NSec]]]];
  ...
  Write[ToString[adatafile]<>"_Ser.dat",collectedtotal];
  Close[ToString[adatafile]<>"_Ser.dat"];
...
(******************************************************************************)
  DirSeries[aname1_,aname2_]:=Module[{aux1,aux2,ic,jc,initialdir},
  ...
  If[
     ...
     StringCases[aux2,ToString[aux1[[jc]]]<>"_Weight.dat"]=!={},
       ...
       Print["Something went wrong on DirSeries"]
...
(************************)
  If[LengthWei===1,
    ...
    1*EpAzimuth,{-1,1}]/.{ST5->(Sin[Pi x5])^(2) };
...
If[LengthWei===2,
  ...
  simple=simple/.{Int[f_]:>Int[GroupLogs[f]]};
    ...
    expandedints={AlphaEpsilonExpandMod[(simple[[1]]),1*EpAzimuth,{-1,1}],
    AlphaEpsilonExpandMod[(simple[[2]]),1(-ep)(*/EpAzimuth*),{-1,1}]}/.{ST5->(Sin[Pi x5])^(2) } ;
