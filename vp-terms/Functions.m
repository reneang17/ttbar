...
(*******************************************************************************
 * 
 *  Integrate NNLO integral at the boundary -- special case
 *
 ******************************************************************************)

ProcessIntegralNNLOSpecial[integral_] := Module[{tmp, ReplChangeVar, jac, 
                                          intx, inty, 
...
remainder, dist, arg},
  ...
  If[Length[distarg]==0,
    ...
    (* case with a distribution *)
      ...
      (* case with star[] *)
        If[dist==star,
           CPrint[FF[arg (func - ReplaceAll[func, x -> 0]), lim(*, GenerateConditions -> False, Assumptions -> ass*)]];
           res = NNIntegrate[arg (func - ReplaceAll[func, x -> 0]), lim , AccuracyGoal->5]; 
         ];
      ...
      (* case with plus[] *)
        If[dist==plus,
           (* first part with subtraction *)
           mainpart = NNIntegrate[arg (func - ReplaceAll[func, x -> 1]), lim, AccuracyGoal->5]; 
           CPrint[FF[arg (func - ReplaceAll[func, x -> 1]), lim]];
				 
           (* remainder with integrated plus argument *)
           remainder = ReplaceAll[func, x -> 1] 
                       Simplify[NNIntegrate[arg, {x, 0, xlo}, AccuracyGoal->5](*, Assumptions->ass*)];
           CPrint[FF[arg, {x, 0, xlo}]];
           res = mainpart - remainder;
           ];
  ];
